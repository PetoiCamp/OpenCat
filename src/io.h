...
void read_serial() {
  if (Serial.available() > 0) {
    ...
    } while (newCmd[cmdLen - 1] != terminator && long(millis() - lastTime) < SERIAL_TIMEOUT);  //the lower case tokens are encoded in ASCII and can be entered in Arduino IDE's serial monitor
